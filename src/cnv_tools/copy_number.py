...
class CopyNumber(ABC):
    ...
    def copy_number_preprocess(
        ...
        if isinstance(copy_number_data, pl.DataFrame):
            copy_number_lf: pl.LazyFrame = copy_number_data.lazy()
        elif isinstance(copy_number_data, pl.LazyFrame):
            copy_number_lf: pl.LazyFrame = copy_number_data
        else:
            raise TypeError(
        ...
        copy_number_lf = copy_number_lf.filter(
            pl.col("chr").is_in(chromosome_names)
        ).sort(
            [
                pl.col("chr")
                .replace(
    ...
    def chromosome_copy_number(
        ...
        chromosome_copy_number : polars.DataFrame | polars.LazyFrame
            A polars DataFrame or LazyFrame that contains the following columns: {"chr": polars.String, "copy_number": polars.Float32, "integer_copy_number":pl.Int32}. Other columns will not be modified.
        """
        chromosome_copy_number: PolarsFrame = (
            copy_number.select(pl.col(["chr", "copy_number"]))
